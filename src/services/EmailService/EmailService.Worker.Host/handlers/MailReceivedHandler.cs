...
namespace EmailService.Worker.Host.handlers
{
    public class MailReceivedHandler
    {
        ...
        public MailReceivedHandler(ILogger<MailReceivedHandler> logger, IMailBoxService mailBoxService, ICustomerSdkService customerSdkService, IMailManagementController mailManagementController, IEmailTicketMessageRepository emailTicketMessageRepository, IMessageSdkService messageSdkService, ITicketService ticketService)
        {
            _logger = logger;
        ...
        public async Task Handle(mail_received mail_Received)
        {
            ...
            var senderName = fullEmail.From.Mailboxes.First().Name;
        ...
        public async Task<Result<(int TicketId, Guid CustomerId)>> NewEmailRecivedFlow(string SenderEmail, string SenderName, string Subject, Guid TenantId)
        {
            try
            {
                Guid CustomerId = Guid.Empty;
            ...
            {
                _logger.LogError(ex, "Error processing new incoming email. Subject: {Subject}, From: {From}", message.Subject, message.From);
